...
if credentials:
    ...
    if not df.empty:
        ...
        if "station" in query_params and "asset" in query_params:
            modal_station = query_params["station"]
            modal_asset = query_params["asset"]
        ...
        for tab, (tab_name, station_value) in zip(tabs, stations.items()):
            with tab:
                ...
                if f'modal_{station_key}' in st.session_state:
                    modal_asset = st.session_state[f"modal_{station_key}"]
                    modal_data = st.session_state[f"modal_data_{station_key}"]

                    st.markdown(
                        f'<div class="modal-header">{modal_asset} '
                        f'<span class="modal-count">({len(modal_data)} items)</span></div>',
                        unsafe_allow_html=True,
                    )
                    ...
                    for idx, row in modal_data.iterrows():
                        asset_number = row.get(asset_name_col, 'N/A')
                        with st.expander(asset_number):
                            col1, col2, col3 = st.columns(3)
                            with col1:
                                st.markdown("**Type**")
                                st.write(row.get(df.columns[1], "N/A"))
                                st.markdown("**Quantity**")
                                st.write(row.get(df.columns[3], "N/A"))
                            with col2:
                                st.markdown("**Dimensions**")
                                dims = f"{row.get(df.columns[4], 'N/A')} × {row.get(df.columns[5], 'N/A')} × {row.get(df.columns[6], 'N/A')} cm"
                                st.write(dims)
                                st.markdown("**Voltage**")
                                st.write(row.get(df.columns[7], "N/A"))
                            with col3:
                                st.markdown("**Power**")
                                st.write(row.get(df.columns[8], "N/A"))
                                st.markdown("**Status**")
                                st.write(row.get(df.columns[9], "N/A"))

                # --- CARD VIEW ---
                else:
                    grouped = station_df.groupby(asset_name_col)
                    asset_groups = list(grouped)
                    num_cols = 4

                    for i in range(0, len(asset_groups), num_cols):
                        cols = st.columns(num_cols)
                        for col_idx, (asset_name, group_df) in enumerate(asset_groups[i:i + num_cols]):
                            with cols[col_idx]:
                                count = len(group_df)
                                safe_name = f"{station_key}_{i}_{col_idx}"
                                ...
                                if st.button(" ", key=f"{safe_name}_{asset_name}", use_container_width=True):
                                    ...
                                    st.query_params["station"] = station_value
